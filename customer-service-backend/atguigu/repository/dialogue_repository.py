...
class DialogueRepository:
    ...
    async def delete_turn(self, sender_id: str, turn_id: str):
        """删除指定用户的某一轮对话记录。"""
        state = await self.load_state(sender_id)
        for session in state.sessions:
            remaining = [turn for turn in session.turns if turn.turn_id != turn_id]
            if len(remaining) != len(session.turns):
                session.turns = remaining
                break
        await self.save_state(sender_id, state)
