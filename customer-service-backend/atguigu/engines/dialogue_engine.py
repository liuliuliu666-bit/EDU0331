...
class DialogueEngine:
    ...
    async def _handle_text_message(self, dialogue_state: DialogueState) -> list[BotMessage]:
        ...
        if not validated.valid:
            tracelog.trace("clarify", reason=validated.reason.value if validated.reason else None)
    ...
    async def _handle_object_message(self,
        ...
        tracelog.trace(
            ...
            object_id=object.id,
            set_slots_command=command.command if command is not None else None,
        )
        ...
        # 情况1:澄清
        tracelog.trace("clarify", reason=ClarifyReason.OBJECT_REQUIRES_INTENT.value)
        return await self.clarify_responder.respond(reason=ClarifyReason.OBJECT_REQUIRES_INTENT,
                                                    dialogue_state=dialogue_state)

    def _trace_route_task(self, turn_plan, dialogue_state: DialogueState) -> None:
        """记录 task 轨道的命令概览，用于审计与问题定位。"""
        try:
            commands = turn_plan.task.commands
            commands_summary = []
            for command in commands:
                if hasattr(command, "flow"):
                    commands_summary.append(f"{command.command}:{command.flow}")
                else:
                    commands_summary.append(command.command)
            tracelog.trace("route", track="task", commands=commands_summary)
        except Exception:
            tracelog.trace("route", track="task")
    ...
    def _try_build_set_slots_command(self,
        ...
        """
        职责：根据卡片对象类型映射到对应槽位。
        支持：订单卡片 -> order_number；班次卡片 -> cohort。
        Args:
            dialogue_state:
            flow_list:
        ...
        slot_name = object_slot_mapping.get(object.type)
