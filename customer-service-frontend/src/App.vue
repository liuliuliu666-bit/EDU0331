    ...
    if (message.role === 'user') {
      ...
      if (!currentTurn) {
        ...
        currentTurn = {
          ...
          id: `turn-${turnIndex}`,
          index: turnIndex,
          turnId: message.turn_id ?? currentTurn?.turnId ?? null,
          userMessage: null,
          botMessages: []
        }
      }
      if (!currentTurn.turnId && message.turn_id) {
        currentTurn.turnId = message.turn_id
      }
...
const ordersEndpoint = computed(() => '/api/v1/orders')
const cohortsEndpoint = computed(() => '/api/v1/me/cohorts')
...
async function confirmDeleteTurn(turnId) {
  if (!turnId) return
  if (!window.confirm('确认删除这轮对话吗？删除后将无法恢复。')) return
  ...
  try {
    const response = await fetch(
      `/api/chat/turn?sender_id=${encodeURIComponent(currentSenderId)}&turn_id=${encodeURIComponent(turnId)}`,
      { method: 'DELETE' },
    )
    if (!response.ok) {
      throw new Error('删除这轮对话失败。')
    }
    resetConversation()
    await fetchChatHistory()
  } catch (error) {
    errorMessage.value = error instanceof Error ? error.message : '删除这轮对话失败。'
  }
}
...
function toStatusLabel(code) {
  if (!code) return '未知'
  return EDU_STATUS_LABEL[code] || code
}

function getStatusClass(status) {
  return EDU_STATUS_CLASS[status] || 'status-muted'
}
...
function formatObjectAmount(payload, type) {
  const amount =
    type === 'order'
      ? payload.paidAmount ??
        ...
        payload.attributes?.paidAmount
      : payload.salePrice ??
        payload.price ??
        payload.attributes?.salePrice ??
        payload.attributes?.price
  return amount !== undefined && amount !== null ? formatAmount(amount) : ''
}
...
function getObjectTitle(message) {
  ...
  return (
    ...
    payload.cohortName ||
    payload.seriesName ||
    objectTypeLabel(message.objectType)
  )
}
...
function getObjectIdentifier(message) {
  ...
  if (type === 'order') {
    const id = payload.orderNo ?? payload.order_id ?? payload.id
    return id ? `订单号：${id}` : '订单号'
  }
  if (type === 'cohort') {
    const id = payload.cohortId ?? payload.id
    return id ? `班次号：${id}` : '班次号'
  }
  if (type === 'series') {
    const id = payload.seriesId ?? payload.id
    return id ? `课程号：${id}` : '课程号'
  }
  return payload.id ? `编号：${payload.id}` : '编号'
}
...
function getObjectSummary(message) {
  ...
  const type = message.objectType
  if (type === 'order') {
    const status = payload.orderStatusCode ?? payload.status ?? payload.attributes?.status
    return status ? `订单状态：${toStatusLabel(status)}` : '订单'
  }
  if (type === 'cohort') {
    const series = payload.seriesName ?? payload.attributes?.seriesName
    const status = payload.enrollStatusCode ?? payload.attributes?.enrollStatusCode
    const parts = [series, status ? `状态：${toStatusLabel(status)}` : ''].filter(Boolean)
    return parts.join('，') || '班次'
  }
  return payload.description || objectTypeLabel(type)
}

function getObjectAmount(message) {
  return formatObjectAmount(message.payload ?? {}, message.objectType)
}
...
async function fetchSidebarData() {
  ...
  try {
    const [ordersResponse, cohortsResponse] = await Promise.all([
      fetch(ordersEndpoint.value, { headers: { 'X-User-Id': currentSenderId } }),
      fetch(cohortsEndpoint.value, { headers: { 'X-User-Id': currentSenderId } }),
    ])

    const [ordersPayload, cohortsPayload] = await Promise.all([
      ordersResponse.json(),
    ...
    if (!cohortsResponse.ok) {
      throw new Error(cohortsPayload.detail || '加载班次列表失败。')
    }
    ...
    orders.value = orderList.map(_orderCard)
    cohorts.value = cohortList.map(_cohortCard)
  } catch (error) {
...
function _orderCard(item) {
  const firstItem = item.orderItems?.[0] ?? {}
  const orderNo = item.orderNo ?? item.orderId
  const courseName = item.courseName || firstItem.itemName || orderNo
  return {
    id: orderNo,
    order_id: orderNo,
    ...
    title: courseName,
    courseName,
    status: toStatusLabel(item.orderStatusCode),
    orderStatusCode: item.orderStatusCode,
    amount: item.paidAmount ?? item.payableAmount,
...
function _cohortCard(item) {
  return {
    id: item.cohortId,
    cohortId: item.cohortId,
    cohortName: item.cohortName,
    seriesName: item.seriesName,
    title: `${item.seriesName} · ${item.cohortName}`,
    status: toStatusLabel(item.enrollStatusCode),
    enrollStatusCode: item.enrollStatusCode,
    enrollAt: item.enrollAt,
  }
}
...
async function sendOrder(order) {
  ...
  await sendPayload({
    object: {
      type: 'order',
      id: String(order.orderNo || order.order_id),
      title: order.title,
      attributes: {
        status: order.status,
        amount: order.amount,
  ...
  await sendPayload({
    object: {
      ...
      attributes: {
        ...
        seriesName: cohort.seriesName,
        enrollStatusCode: cohort.enrollStatusCode,
      },
    },
  })
}
        ...
        <section ref="messagesContainer" class="messages">
          <div v-if="turns.length === 0" class="welcome">
            <div class="welcome-card">
              ...
              <p class="welcome-subtitle">你的专属教育智能客服，随时为你服务</p>
              <div class="welcome-chips">
                <button
                  v-for="chip in ['我要退款', '查订单状态', '查学习进度', '咨询课程', '提交工单']"
                  :key="chip"
                  type="button"
                  class="welcome-chip"
                  :disabled="isSending"
            ...
            <div v-else class="turn-card" :class="{ 'has-user-message': item.userMessage }">
              ...
              <div class="turn-header">
                ...
                <button
                  ...
                  class="turn-delete"
                  title="删除这轮对话"
                  @click="confirmDeleteTurn(item.turnId)"
                >🗑️</button>
              </div>
                  ...
                  <template v-if="item.userMessage.type === 'object'">
                      <div class="object-card" :class="`object-card-${item.userMessage.objectType}`">
                        <div class="object-card-badge">
                          {{ objectTypeLabel(item.userMessage.objectType) }}
                        </div>
                      <img
                  ...
                  >
                    <template v-if="botMsg.type === 'object'">
                      <div class="object-card" :class="`object-card-${botMsg.objectType}`">
                        <div class="object-card-badge">
                          {{ objectTypeLabel(botMsg.objectType) }}
                        </div>
                        <img
      ...
      <aside class="sidebar">
        ...
        <div v-else class="sidebar-list">
          <div v-if="!cohorts.length && !isLoadingSidebar" class="sidebar-empty">
            暂无班次数据（需 X-User-Id 对应当前学员）
          </div>

          <article v-for="cohort in cohorts" :key="cohort.cohortId" class="sidebar-card">
            <div class="card-image-wrapper">
              <div class="card-image-placeholder">🎓</div>
            </div>
            <div class="card-top">
              <div class="card-title">{{ cohort.title }}</div>
              <div class="card-amount">{{ cohort.enrollAt ? '已报名' : '' }}</div>
            </div>
            <div class="card-meta">班次号：{{ cohort.cohortId }}</div>
            <div class="card-meta">
              <span class="status-badge" :class="getStatusClass(cohort.status)">{{ cohort.status }}</span>
            </div>
            <button
              type="button"
              class="secondary-button full-width"
...
.turn-delete {
  ...
  font-size: 14px;
  cursor: pointer;
  transition: color var(--duration-fast) var(--ease-in-out), transform var(--duration-fast) var(--ease-out-expo);
}
.turn-delete:hover {
  color: var(--color-danger);
